...
for k in $(seq 0 $((${#SERVERS[@]}-1))); 
	do
		cmd="sudo mv /etc/riak/riak.conf.bak /etc/riak/riak.conf && sudo sed 's/riak@127.0.0.1/riak@${SERVERS[k]}/;s/## \(strong_consistency\)/\1/' -i.bak /etc/riak/riak.conf && sudo riak start"
