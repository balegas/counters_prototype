...
reset_bucket(NKeys,InitValue,Bucket, RiakAddress, RiakPort) ->
  lists:foreach(fun(Key)-> reset_crdt(InitValue,Bucket,integer_to_binary(Key),RiakAddress,RiakPort) end,
    lists:seq(0,NKeys)).
...
reset_crdt(InitValue, Bucket, Key, RiakAddress, RiakPort) ->
  ...
  case Result of
    {ok, Value} ->
      riakc_pb_socket:counter_incr(Pid,Bucket, Key,(InitValue-Value),[{w,?REPLICATION_FACTOR}]);
    {error,_} ->
