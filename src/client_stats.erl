  ...
  {Pid, Key, Value, Permissions, Latency, Timestamp, Operation, Status} ->
    ...
    case Status of
      ...
      failure ->
        io:fwrite(File, "~p\t~p\t~p\t~p\t~p\t~p\t~p\t~p\t~n", 
                  [Pid,Key,Value, Permissions, Latency, now_microsec(Timestamp), Operation, Status]),
        stats(NewBins,Success,Fail+1,Forbidden,Finished, min(Value,MinValue),
              StartTime,Running,Total,File,ParentPid);
      forbidden ->
        io:fwrite(File, "~p\t~p\t~p\t~p\t~p\t~p\t~p\t~p\t~n", 
                  [Pid,Key,Value, Permissions, Latency, now_microsec(Timestamp), Operation, Status]),
        stats(NewBins,Success,Fail,Forbidden+1,Finished, min(Value,MinValue),
              StartTime,Running,Total,File,ParentPid);
      finished ->
        io:fwrite(File, "~p\t~p\t~p\t~p\t~p\t~p\t~p\t~p\t~n", 
                  [Pid,Key,Value, Permissions, Latency, now_microsec(Timestamp), Operation, Status]),
        stats(NewBins,Success,Fail,Forbidden,Finished+1, min(Value,MinValue),
              StartTime,Running,Total,File,ParentPid)
    end;
  start ->
...
print_output(Bins,Success,Fail,Forbidden,Finished,StartTime,Total) ->
  io:format("T\tAVG_L\t~n"),
  {TotalLatency,TotalOps} = orddict:fold(fun(K,{Sum,Count},{AccSum,AccCount}) ->
    if(Count > 0) ->
      io:format("~p\t~p~n",[K,Sum/Count/?TIME_UNIT]);
    true -> ok
    end,
    {AccSum+Sum,AccCount+Count} end,{0,0},Bins),
